...
public class ConformanceChecker {
  ...
  static int readNumber(ByteBuffer input, int maxBits) {
    int bits = (0xFFFF & input.getShort()) % (maxBits + 1);
    if (bits == 0) return 0;
  ...
  static void checkRequest(ByteBuffer input) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < numArguments; ++i) {
        origBuilder.addArguments(new String(makeBlob(rng, input, 15), StandardCharsets.UTF_8));
      }
      int numInputs = readNumber(input, 8);
      for (int i = 0; i < numInputs; ++i) {
        WorkerProtocol.Input.Builder inputBuilder = WorkerProtocol.Input.newBuilder();
        inputBuilder.setPath(new String(makeBlob(rng, input, 15), StandardCharsets.UTF_8));
  ...
  static void checkResponse(ByteBuffer input) throws IOException {
    ...
    try {
      ...
      orig.output = new String(makeBlob(rng, input, 22), StandardCharsets.UTF_8);
    } catch (BufferUnderflowException ex) {
      // Okay, input is too short.
      return;
    }
